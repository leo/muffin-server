...
router.get('/', function *() {
  try {
    ...
    console.error('Couldn\'t find file', err)
  }

  if (!results) {
    this.status = 500
    this.body = 'Couldn\'t find file'
...
router.post('/upload', function *(next) {
  ...
  do {
    ...
    try {
      ...
      console.error('Couldn\'t check if media exists', err)
      return
    }
  ...
  try {
    ...
    console.error('Not able to save media', err)
    return
  }
